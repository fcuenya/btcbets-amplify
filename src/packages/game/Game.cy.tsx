...
describe(
  ...
  () => {
    ...
    it("should start processing a guess once the 'lower' or 'higher' buttons are pressed", () => {
      ...
      cy.get(".radial-progress .price").contains("--").should("not.exist");
      cy.get(".radial-progress .bet")
        .contains("is LOWER than")
        .should("not.exist");
      cy.get("button").contains("Lower").click();
      cy.get(".radial-progress .bet").contains("is LOWER than").should("exist");
